...
def geo_position(x_bari, y_bari, x, y):
    ...
    if y <= ((max_y - min_y)/(min_x - max_x))*(x-x_bari)+y_bari and y > ((min_y-max_y)/(min_x-max_x))*(x-x_bari)+y_bari:
        return "n"
...
among_closest = 15
connectors_parameters = {"function" : 1,
                         "lanes" : 5,
                         "speed" : 200,
                         "capacity" : 99999}
...
for _, r in df_zones.iterrows():
    ...
    df_intersections_["dist"] = haversine_np(r["x"], r["y"], df_intersections_["x"], df_intersections_["y"])
    ...
    df_intersections_["4_zones"] = df_intersections_.apply(lambda x: geo_position(r["x"], r["y"], x["x"], x["y"]), axis=1)
